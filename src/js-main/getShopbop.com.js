...
CDP(async chrome => {
    chrome.Page.enable();
    let stream = fs.createWriteStream("E:/workspace/brandcrawler/src/main/resources/cn.shopbop.com_brand_result.txt",
        { 'flags': 'a'});
                  ...
                  .then(() => {
                      chrome.once('Page.loadEventFired', () => {
                          chrome.DOM.getDocument().then((node) => {
                              ...
                              dataObj.brandDescriptionCn = root("#bioCopy").text().trim();
    ...
    let fileData = fs.readFileSync('E:/workspace/brandcrawler/src/main/resources/cn.shopbop.com_brand.txt', 'utf8');
