...
CDP(async chrome => {
    chrome.Page.enable();
    let stream = fs.createWriteStream("E:/workspace/brandcrawler/src/main/resources/sephora.fr_brand_有描述_result.txt",
        { 'flags': 'a'});
                  ...
                  .then(() => {
                      chrome.once('Page.loadEventFired', () => {
                          chrome.DOM.getDocument().then((node) => {
                              ...
                              dataObj.brandLogoSrc = root("#fondLogoMarques img.logoMarques").attr("src");
                              if (dataObj.brandLogoSrc) {
                                  dataObj.brandLogoSrc = "https:/" + dataObj.brandLogoSrc;
                                  dataObj.brandLogoSrc = dataObj.brandLogoSrc.replace("//media", "//www.sephora.fr/media");
                              }
                              dataObj.brandDescription = root("#fondLogoMarques div.desc").text().trim();
    ...
    let fileData = fs.readFileSync('E:/workspace/brandcrawler/src/main/resources/sephora.fr_brand_有描述.txt', 'utf8');
